...
void Merge2AscListsRecur(Node*& xList, Node*& yList, Node*& zList)
{
   ...
   if(zList != nullptr && zList->link != nullptr && zList->data > zList->link->data)
   {
      Node* temp = zList;
      zList = zList->link;
      temp->link = zList->link;
      zList->link = temp;
   }
   Merge2AscListsRecur(xList, yList, zList->link);
}
